...
@ tra e bss va dichiarato tutto ciò che verrà usato
...
ind_c:  .space 4    @ mi "unlock" una word, ovvero 4 byte (in compilazione questi 4 byte non verranno mai usati)

.text

@ quello che c'è dopo questa istruzione va codificato
...
@ main sarà un indirizzo di memoria: definisce una label "etichetta" che si chiama main (quella dopo i :), ovvero un'istruzione definita da 32 bit
...
@ loop: b loop          @ loop infinito, b significa "salta". Dove? Di nuovo a questa istruzione
        ...
        @ mov r0, r1    @ questa istruzione non verrà mai eseguita a causa del loop all'istruzione precedente
        ...
        @ bGT positivo    @ esegui questa istruzione solo se r3 > 0 --> il PC viene aggiornato a positivo
        @ questo b in realtà non serve (lo commento per fare vedere come si dovrebbe fare per esteso)

        bLE nonpositivo         @ esegui questa istruzione solo se r3 <= 0 --> il PC viene aggiornato a nonpositivo
...
nonpositivo:    mov r4, #10     @ metto in B (r4) il valore 10
                                @ non serve nessun codice di uscita perché se viene eseguito questo codice, l'istruzione successiva è fuori dall'if
...
@ ciò che inizia con il . sono direttive per il compilatore
